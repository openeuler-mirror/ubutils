...
void port_cap2_link_log(uint8_t *data, uint32_t data_len)
{
    ...
    static struct val_desc err_log_desc[] = { { 0x0, "Flit Error Counter" }, { 0x1, "LTB Error Log 1" },
                                              { 0x2, "LTB Error Log 2" }, { 0x3, "LTB Error Log 3" },
                                              { 0x4, "LTB Error Log 4" }, { INVALID_DESC, "" } };

    slice_ver = slice_get_version(data);
    slice_size = slice_get_size(data);
    sprintf(port_info->display_buf, "\n\t\tPORT_CAP2_LINK_LOG: slice[0x%x, 0x%x] id[%u]", slice_ver, slice_size,
            port_cap_id);
    ...
    off = sprintf(port_info->display_buf,
                  "\n\t\t\tFlit Error Log 1: Flit Err Log Valid%s"
                  "\n\t\t\tFlit Error Counter Control: Flit Error Counter Enable%s",
