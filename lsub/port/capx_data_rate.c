...
static void port_capx_data_rate_sta(uint8_t *data)
{
    ...
    off += sprintf(port_info->display_buf + off,
                   "\n\t\t\t\tEqualization Complete%s Equalization Coarsetune phase Complete%s"
                   "\n\t\t\t\tEqualization Coarsetune Phase Successful%s Equalization Passive Phase Successful%s",
    ...
    off += sprintf(port_info->display_buf + off,
                   "\n\t\t\t\tEqualization Active Phase Successful%s Link Equalization Request%s"
    ...
    off += sprintf(port_info->display_buf + off,
                   "\n\t\t\t\tCurrent FEC Mode: %s, Current BCRC Mode: %s"
                   "\n\t\t\t\tCurrent FEC Interleave state:%s",
                   get_description(tmp0, fec_desc), get_description(tmp1, bcrc_desc),
                   bit_parser(btmp0));

    off += sprintf(port_info->display_buf + off, "\n\t\t\t\tCur Local Port Transmitter Preset value Lane0~7(hex)"
                                                 "\n\t\t\t\t\t");
    cur_data += CFG_DWORD_LEN;
    for (i = 0; i < CFG_DWORD_LEN; i++) {
        off += sprintf(port_info->display_buf + off, " %x %x", (uint32_t)(*cur_data & 0xf), (uint32_t)(*cur_data >> CFG_BIT4));
...
void port_capx_data_rate(uint8_t *data, uint32_t data_len)
{
    ...
    sprintf(port_info->display_buf, "\n\t\tport data rate: slice[0x%x, 0x%x] id[%u]", slice_ver, slice_size,
            port_cap_id);
    printf("%s", port_info->display_buf);
