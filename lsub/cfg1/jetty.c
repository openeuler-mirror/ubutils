...
void cfg1_jetty_cap(uint8_t *data, uint32_t data_len)
{
    ...
    uint8_t *jetty_data;
    uint16_t eventq_num;
    uint8_t jetty_max_num, jfc_max_num, jfr_max_num;
    uint8_t slice_ver;
    uint32_t slice_size;
    int off;
    ...
    off = sprintf(cfg1_info->display_buf,
        "\n\t\tCFG1_CAP2_JETTY: slice[0x%x, 0x%x] id[%d]",
    ...
    off += sprintf(cfg1_info->display_buf + off,
        "\n\t\t\tJFS and Jetty Max Number: %s\n\t\t\tJFC Max Number: %s"
        "\n\t\t\tJFR Max Number: %s\n\t\t\tEvent Queue Number: %u",
        jfx_max_num_desp[jetty_max_num], jfx_max_num_desp[jfc_max_num],
